...
class ISM(object):
            ...
            for ii, freq in enumerate(self.freq_Array):
                ...
                if (ii+1) % int(self.Nf//20) ==0:
                    ...
                    try: #Python 2 workaround. Python 2 __future__ does not have 'flush' kwarg.
                        print('\r{0:2.0f}% FD shifted in {1:4.3f} seconds.'.format(round((ii + 1)*100/self.Nf) , shift_check-shift_start), end='', flush=True)
                    except: #This is the Python 2 version.
                        print('\r{0:2.0f}% FD shifted in {1:4.3f} seconds.'.format(round((ii + 1)*100/self.Nf) , shift_check-shift_start), end='')
                    sys.stdout.flush()
    ...
    def FD_Shift(self):
        ...
        k = 0
        for FD in self.FDs:
            # Should be natrual log not log10 -> yes
            self.FD_delay += np.double(-1.0*FD * \
                    np.power(np.log(self.freq_Array/1000.0),k+1)) # will be in seconds
            #self.FD_delay += np.double(FD) * \
            #        np.power(np.double(np.log(np.double(self.freq_Array)/np.double(1000.0))),np.double(k+1)) # will be in seconds
            k += 1
